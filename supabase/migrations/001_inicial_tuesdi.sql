...
-- POLÍTICAS
CREATE POLICY "Artistas públicos son visibles para todos" ON public.artists FOR SELECT USING (true);
CREATE POLICY "Artistas pueden actualizar su perfil" ON public.artists FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Usuarios autenticados pueden crear artistas" ON public.artists FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Artistas pueden ver sus solicitudes" ON public.contact_requests FOR SELECT USING (artist_id IN (SELECT id FROM public.artists WHERE user_id = auth.uid()));
CREATE POLICY "Cualquiera puede enviar solicitudes" ON public.contact_requests FOR INSERT WITH CHECK (true);

CREATE POLICY "Artistas pueden ver sus métricas" ON public.metrics FOR SELECT USING (artist_id IN (SELECT id FROM public.artists WHERE user_id = auth.uid()));

CREATE POLICY "Artistas pueden ver su suscripción" ON public.subscriptions FOR SELECT USING (artist_id IN (SELECT id FROM public.artists WHERE user_id = auth.uid()));

CREATE POLICY "Media de artistas es pública" ON public.media FOR SELECT USING (true);
CREATE POLICY "Artistas pueden gestionar su media" ON public.media FOR ALL USING (artist_id IN (SELECT id FROM public.artists WHERE user_id = auth.uid()));

CREATE POLICY "Eventos aprobados son públicos" ON public.events FOR SELECT USING (status = 'approved' OR status = 'pending');
CREATE POLICY "Cualquiera puede crear eventos" ON public.events FOR INSERT WITH CHECK (true);
...
SELECT
    ...
    a.profile_image, a.subscription_plan, a.verified, a.created_at, a.updated_at,
